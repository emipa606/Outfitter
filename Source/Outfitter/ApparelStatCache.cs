...
public class ApparelStatCache
{
    ...
    public float ApparelScoreRaw([NotNull] Apparel ap)
    {
        ...
        if (ap.def.HasComp(typeof(CompShield)) && _pawn.equipment.Primary?.def.IsRangedWeapon == true)
        {
            return -1f;
        }
    ...
    public float ApparelScoreRaw_Temperature([NotNull] Apparel apparel)
    {
        ...
        var log =
            $"{apparel.LabelCap} - InsCold: {insulationCold} - InsHeat: {insulationHeat} - TargTemp: {targetTemperatures}\nMinComfy: {minComfyTemperature} - MaxComfy: {maxComfyTemperature}";
        ...
        log +=
            $"\nWeight: {tempWeight} - NeedInsCold: {neededInsulationCold} - NeedInsWarmth: {neededInsulationWarmth}";
        ...
        log +=
            $"\nScoreOffsetMin: {temperatureScoreOffset.min} - ScoreOffsetMax: {temperatureScoreOffset.max} *= {temperatureScoreOffset.min * temperatureScoreOffset.max}";
