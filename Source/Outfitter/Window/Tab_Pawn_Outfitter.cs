...
public class Tab_Pawn_Outfitter : ITab
{
    ...
    private void DrawApparelStats(Vector2 cur, Rect canvas)
    {
        ...
        else
        {
            ...
            GUI.color = Color.grey;
            Text.Anchor = TextAnchor.LowerLeft;
            Widgets.Label(legendRect, $"-{ApparelStatCache.MaxValue:N1}");
    ...
    private void InterfaceDrop([NotNull] Thing t)
    {
        switch (t)
        {
            ...
            case ThingWithComps thingWithComps
                when SelPawn.equipment.AllEquipmentListForReading.Contains(
                    thingWithComps):
    ...
    private void TryDrawComfyTemperatureRange(ref float curY, float width)
    {
        ...
        Widgets.Label(
            rect,
            $"{"ComfyTemperatureRange".Translate()}: {statValue.ToStringTemperature("F0")} ~ {statValue2.ToStringTemperature("F0")}");
        curY += 22f;
    }
}